...
async def send_handover_slack_dm(
    ...
    *,
    ...
    markdown = handover.markdown_content or handover.markdown
    filename = handover.filename if handover.filename.endswith(".md") else f"{handover.filename}.md"
    ...
    initial_comment = (
        f"Empulse handover blueprint for *{handover.employee_name}* is attached. "
    ...
    file_id = upload_markdown_to_channel(
        ...
        filename=filename,
        content=markdown,
        title=f"Handover — {handover.employee_name}",
